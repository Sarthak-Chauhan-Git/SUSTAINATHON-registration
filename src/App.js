...
class RegistrationForm {
  ...
  populateDropdowns() {
    ...
    const pillarSelect = document.querySelector(
      'select[name="innovativePillar"]'
    );
    if (pillarSelect) {
  ...
  setupEventListeners() {
    ...
    const extraMembersRadios = document.querySelectorAll(
      'input[name="hasExtraMembers"]'
    );
    ...
    const addMoreRadios = document.querySelectorAll(
      'input[name="addMoreMembers"]'
    );
  ...
  handleFileSelect(input, file, uploadContainer) {
    ...
    if (file.size > maxSize) {
      this.showNotification(
        `File size exceeds ${uploadContainer.dataset.maxSize}MB limit`,
        "error"
      );
      return;
    }
  ...
  nextStep() {
    if (this.validateCurrentStep()) {
      ...
      if (this.currentStep === 3) {
        const hasExtraMembers = document.querySelector(
          'input[name="hasExtraMembers"]:checked'
        )?.value;
  ...
  validateCurrentStep() {
    ...
    const requiredFileUploads = currentStepElement.querySelectorAll(
  ...
  saveCurrentStepData() {
    const currentStepElement = document.querySelector(".form-step.active");
    const formElements = currentStepElement.querySelectorAll(
  ...
  addMember() {
    ...
    const memberHtml = `
            <div class="additional-member" data-member="${this.memberCount}">
                <h3>Member ${
                  this.memberCount
                } <button type="button" class="btn btn--secondary" onclick="registrationForm.removeMember(${
      this.memberCount
    })" style="font-size: 12px; padding: 4px 8px; margin-left: 16px;">Remove</button></h3>
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" name="member${
                      this.memberCount
                    }Email">
                </div>
                <div class="form-group">
                    <label class="form-label">Phone Number</label>
                    <input type="tel" class="form-control" name="member${
                      this.memberCount
                    }Phone">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="file-upload" data-max-size="10" data-accept="image/*,application/pdf">
                        ...
                        <input type="file" class="file-input" accept="image/*,application/pdf" name="member${
                          this.memberCount
                        }IdCard">
                ...
                <div class="form-group">
                    ...
                    <div class="file-upload" data-max-size="10" data-accept="image/*">
                        ...
                        <input type="file" class="file-input" accept="image/*" name="member${
                          this.memberCount
                        }Photo">
                        <div class="file-preview"></div>
                    </div>
    ...
    const newMember = container.querySelector(
      `.additional-member[data-member="${this.memberCount}"]`
    );
    ...
    this.showNotification(
      `Member ${this.memberCount} added successfully`,
      "success"
    );
  }
  ...
  removeMember(memberNumber) {
    const memberElement = document.querySelector(
      `.additional-member[data-member="${memberNumber}"]`
    );
    if (memberElement) {
      ...
      if (
        this.memberCount < 5 &&
        document.getElementById("addMoreYes").checked
      ) {
  ...
  showSkipMessage() {
    this.showNotification(
      "You can skip to project details since you selected no additional members",
      "info"
    );
  }
  ...
  showSuccessModal() {
    ...
    const teamId =
      "HACK2025" + Math.random().toString(36).substr(2, 6).toUpperCase();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log(
    "🎯 Keyboard shortcuts enabled: Ctrl/Cmd + Arrow Keys for navigation, Esc to close modals"
  );
});
